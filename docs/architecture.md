...
WebSocket 重连后，先等待本次状态查询的完成消息，再恢复元数据读取；重放的旧状态不能提前触发刷新。再次断线、comm 关闭或视图销毁会取消旧握手，不会重建已有 DDB 会话。
服务器分页查询只接受经过校验的路径、索引和名称；不会把代码提示中的任意表达式自动交给数据库执行。

会话工具栏和 DOS 外层工具栏沿用 Jupyter 的 `ReactiveToolbar`。`guardToolbarDisposal` 按实例保护延迟显示回调，避免关闭标签后调用已销毁的节流器，同时保留原生两次测量，确保后台恢复的标签页正确分配溢出菜单。Jupyter 暂无可等待完成的公开刷新接口，此兼容层对内部 `_resizer` 做运行时检查；升级 Jupyter 时需复测恢复标签、窗口缩放和关闭全部标签。
